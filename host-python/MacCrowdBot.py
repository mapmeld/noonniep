		...
		if(dosend == "TRUE" or dostream == "TRUE"):
			...
			if(dosend == "TRUE"):
				...
				if(outbox == ""):
					...
					urllib.urlopen(appinstance + '/speak',urllib.urlencode({'mail':contact,'id':programid,'data':""}))
				else:
					# send full data report
					urllib.urlopen(appinstance + '/speak',urllib.urlencode({'mail':contact,'id':programid,'data':outbox}))
		else:
